...
{
	this->font.loadFromFile("Resources/Fonts/BRLNSDB.ttf");
}
...
void PlayerGUI::initSprintBar()
{
	this->sprintBar = new gui::ProgressBar(
		1.f, 11.0f, 10.4f, 2.8f,
		this->player->getAttributeComponent()->sprintMax,
		sf::Color::Yellow, 180,
		this->vm, &this->font);
}
...
void PlayerGUI::updateSprintBar()
{
	this->sprintBar->update(this->player->getAttributeComponent()->sprint);
}
...
void PlayerGUI::renderSprintBar(sf::RenderTarget& target)
{
	this->sprintBar->render(target);
}

void PlayerGUI::render(sf::RenderTarget& target)
{
	this->renderHPBar(target);
	this->renderSprintBar(target);
}
